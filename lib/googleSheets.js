...
export function sanitizeTabName(title, startTime) {
  const datePart = startTime
    ? new Date(startTime).toLocaleString("en-US", {
        month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit",
      })
    : "no date set";
  const raw = `${title || "Untitled event"} - ${datePart}`.replace(/[:\\/?*[\]]/g, "-");
 ...
 * { id, title, start_time, sheet_tab_gid }. `persistGid` is called with the
 * new gid the first time a tab is created, so the caller can save it back
 * onto the event row - this file has no knowledge of the events table.
 */
export async function ensureEventTab(event, persistGid) {
  const sheets = sheetsClient();
  const wantedName = sanitizeTabName(event.title, event.start_time);
  ...
  try {
    addResult = await sheets.spreadsheets.batchUpdate({
      spreadsheetId: spreadsheetId(),
      requestBody: { requests: [{ addSheet: { properties: { title: wantedName } } }] },
    });
  } catch (e) {
    ...
    if (String(e.message).includes("already exists")) {
      ...
      addResult = await sheets.spreadsheets.batchUpdate({
        spreadsheetId: spreadsheetId(),
        requestBody: { requests: [{ addSheet: { properties: { title: fallbackName } } }] },
      });
