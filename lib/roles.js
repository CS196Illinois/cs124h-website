...
/** Every non-student role - i.e. course staff. */
export const STAFF_ROLE_IDS = ALL_ROLES.map((r) => r.id).filter((id) => id !== "STUDENT");

/**
 * URL-path roles that double as a group-scoped PM. Web devs are also PMs, so a
 * web dev with a group_number gets the full PM dashboard and the same own-group
 * limits. Without a group, their PM roster is empty; developer tools remain available.
 */
export const PM_VIEW_ROLES = ["pm", "web_dev"];
export const isPmViewRole = (role) => PM_VIEW_ROLES.includes(role);
