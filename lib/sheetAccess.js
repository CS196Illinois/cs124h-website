...
// Same set, but as the URL-path role strings on session.user.role (see
// mapRole() in app/api/auth/[...nextauth]/route.js) - for gating routes/UI
// that only have the signed-in user's session to check, not a DB role id.
export const SHEET_ACCESS_PATH_ROLES = new Set(["course_lead", "head_pm", "lead_web_dev"]);
